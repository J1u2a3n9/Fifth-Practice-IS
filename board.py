...
class Board(object):
    ...
    def transform_to_letter(self,number):
        if number==0:
            return 'A'
        if number==1:
            return 'B'
    ...
    def row(self, row_num):
        if row_num < 0 or row_num >= self._size:
            raise ValueError("row_num must be between 0 and {}.".format(self._size))
        return self._board[row_num, :]

    def col(self, col_num):
        if col_num < 0 or col_num >= self._size:
            raise ValueError("col_num must be between 0 and {}.".format(self._size))
    ...
    def __repr__(self):
        ...
        header = "    " + " ".join(map(lambda i: " {: <2}".format(self.transform_to_letter(i)),range(self._size)))
